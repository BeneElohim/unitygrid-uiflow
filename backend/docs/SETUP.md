...
Quick setup instructions for UnityGrid.

## Configuration Setup

UnityGrid uses a YAML configuration file that should be placed in the **project root directory**.

### Steps

1. **Navigate to project root**:
   ```bash
   cd /path/to/unitygrid
   ```
...
4. **Verify configuration**:
   ```bash
   cd backend
   python -c "from unitygrid.config import get_app_config; print('✓ Config loaded:', get_app_config().models[0].name)"
   ```

## Important Notes

- **Location**: `config.yaml` should be in `unitygrid/` (project root), not `unitygrid/backend/`
...
3. `unitygrid/config.yaml` (parent directory - **recommended location**)

**Recommended**: Place `config.yaml` in project root (`unitygrid/config.yaml`).

## Sandbox Setup (Optional but Recommended)

If you plan to use Docker/Container-based sandbox (configured in `config.yaml` under `sandbox.use: unitygrid.community.aio_sandbox:AioSandboxProvider`), it's highly recommended to pre-pull the container image:
...
cd unitygrid/backend
python -c "from unitygrid.config.app_config import AppConfig; print(AppConfig.resolve_config_path())"
```
