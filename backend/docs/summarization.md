...
UnityGrid includes automatic conversation summarization to handle long conversations that approach model token limits. When enabled, the system automatically condenses older messages while preserving recent context.
...
- **Configuration**: `packages/harness/unitygrid/config/summarization_config.py`
- **Integration**: `packages/harness/unitygrid/agents/lead_agent/agent.py`
